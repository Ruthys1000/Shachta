...
- **`/login`** — מסך כניסה עם סיסמה גלובלית (`APP_PASSWORD`). אין שם משתמש, אין הרשמה. בנוסף יש **קוד הדגמה לצפייה בלבד** (`orly-demo`, קבוע בקוד): מי שנכנס איתו רואה את החשבון האמיתי ויכול לחולל דוגמאות (מבדק/סיפור/שיעור), אבל **שום דבר לא נשמר** — סיום תרגול מוחזר בהצלחה בלי כתיבה, והוספה/עריכה/מחיקה של מילים נחסמות. סשן ההדגמה תקף 48 שעות ותחת תקציב AI יומי מחמיר משלו (`AI_DEMO_DAILY_BUDGET_USD`).
...
| `DIRECT_URL` | חיבור **ישיר (לא-pooled)** ל-Postgres, לשימוש רק ב-`prisma migrate deploy`. על מסד serverless עם pooler (למשל Neon) — כדאי להצביע לכתובת ה-DIRECT/unpooled כדי למנוע timeout של advisory lock (P1002) בזמן deploy. אם לא מוגדר, המיגרציות נופלות חזרה ל-`DATABASE_URL` | לא (מומלץ ל-Neon/פרודקשן) |
...
| `AI_DAILY_BUDGET_USD` | תקרת הוצאה יומית (USD) לקריאות Claude. ברירת מחדל 5 | לא |
| `AI_DEMO_DAILY_BUDGET_USD` | תקרת הוצאה יומית מחמירה (USD) לסשן ההדגמה (קוד `orly-demo`), נמדדת בנפרד. ברירת מחדל 1 | לא |
